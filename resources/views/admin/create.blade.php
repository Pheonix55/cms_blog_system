...
@section('content')
    <main class="window-main">
        <div class="window-main-header">
            ...
            <div class="publish-actions">
                ...
                <button class="button button--publish" id="save-btn">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="window-main-body">
            ...
            <div> <input id="x" type="hidden" name="description">
                ...
                <section class="settings-section">
                    ...
                    <div class="input-group">
                        <label class="input-checkbox">
                            <input type="checkbox" class="input-checkbox-box" name="show_author" checked />
                            <span class="input-checkbox-toggle"></span>
                            <span class="input-checkbox-label">Show author</span>
                        </label>
                        <label class="input-checkbox">
                            <input type="checkbox" class="input-checkbox-box" name="date_published" checked />
                            <span class="input-checkbox-toggle"></span>
                            <span class="input-checkbox-label">Date published</span>
                        </label>
                        <label class="input-checkbox">
                            <input type="checkbox" class="input-checkbox-box" name="date_edited" />
                            <span class="input-checkbox-toggle"></span>
    ...
    <script>
        document.getElementById('save-btn').addEventListener('click', async () => {
            ...
            const img = document.querySelector('.input-image-wrapper img');
            if (img) {
                formData.append('featured_image', img.getAttribute('src'));
            }

            formData.append('show_author', document.querySelector('[name="show_author"]').checked ? 1 : 0);
            formData.append('date_published', document.querySelector('[name="date_published"]').checked ? 1 :
                0);
            formData.append('date_edited', document.querySelector('[name="date_edited"]').checked ? 1 : 0);

            try {
                const response = await fetch("{{ route('admin.blog.store') }}", {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    alert('Save failed: ' + (result.message || 'Validation error'));
                }
            } catch (err) {
                console.error(err);
